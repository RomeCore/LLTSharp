...
namespace LLTSharp.TemplateNodes
{
	...
	public class TextTemplateSequentialNode : TextTemplateNode
	{
		...
		/// </summary>
		public IReadOnlyList<TextTemplateNode> Children { get; set; }
		...
		public override void Refine(int depth)
		{
			...
			for (int i = 0; i < Children.Count; i++)
			{
				...
				{
					var lines = plainTextChild.Text.Split(new[] { "\r\n", "\n", "\r" }, StringSplitOptions.None );

					int startLine = i == 0 && lines.Length > 1 && string.IsNullOrWhiteSpace(lines[0]) ? 1 : 0;
					int endLine = i == Children.Count - 1 && lines.Length > 1 && string.IsNullOrWhiteSpace(lines[lines.Length - 1])
						? lines.Length - 1
						: lines.Length;
			...
			for (int ci = 0; ci < Children.Count - 1; ci++)
			{
			...
			var newChildren = new List<TextTemplateNode>();
			...
			foreach (var child in Children)
			{
				...
				else
				{
					...
					{
						newChildren.Add(new TextTemplatePlainTextNode(childrenSb.ToString()));
						childrenSb.Clear();
					}
