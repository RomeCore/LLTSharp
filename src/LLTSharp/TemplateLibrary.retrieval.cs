...
namespace LLTSharp
{
	...
	{
		private bool TryRetrieve(IMetadata metadata, bool useFallbackSchemes, out List<ITemplate> templates)
		{
			lock (_lockObject)
			{
		...
		private HashSet<ITemplate> Retrieve(IMetadata[] metadatas, bool useFallbackSchemes)
		{
			if (metadatas == null)
		...
		private HashSet<ITemplate>? TryRetrieve(IMetadata[] metadatas, bool useFallbackSchemes)
		{
			if (metadatas == null)
				throw new ArgumentNullException(nameof(metadatas));
			if (metadatas.Length == 0)
